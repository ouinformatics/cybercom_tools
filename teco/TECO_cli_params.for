...
!     scenario
      ...
      character(len=80) writer_str,yr_data_str
      character(len=100) param_file,amb_file,C_file,H2O_file,pools_file
199   format(a80)
...
! open input and output files:
	call getarg(1,yr_data_str)
	call getarg(2,writer_str)
	call getarg(3,amb_file)
	call getarg(4,C_file)
	call getarg(5,H2O_file)
	call getarg(6,pools_file)
!    open(10,file='TECO_param.txt',status='old') !parameters
!    open(10,file=param_file,status='old') !parameters
	open(13, file=amb_file,status='old') !climate data
	read(writer_str,*) writer
	read(yr_data_str,*) yr_data
!	writer=3  !The years to run before recording simulation results
!	yr_data=1 !How many years' data of the climate dataset has
 
!     open output files
      open(61,file=C_file)
      open(62,file=H2O_file)
      open(63,file=pools_file)
...
!     *******************************************************************************
	...
	character(len=80) slat_str,co2ca_str
	character(len=80) ioput_str,stom_n_str,alpha_str
	character(len=80) a1_str,Ds0_str,Vcmx0_str,extkU_str,xfang_str
	character(len=80) wsmax_str,wsmin_str,rdepth_str,rfibre_str
	character(len=80) LAIMAX_str,LAIMIN_str,SLA_str
...
!      close(10)
	call getarg(7,slat_str)
	call getarg(8,co2ca_str)
	call getarg(9,ioput_str)
	call getarg(10,a1_str)
	call getarg(11,Ds0_str)
	call getarg(12,Vcmx0_str)
	call getarg(13,extkU_str)
	call getarg(14,xfang_str)
	call getarg(15,alpha_str)
	call getarg(16,stom_n_str)
	call getarg(17,wsmax_str)
	call getarg(18,wsmin_str)
	call getarg(19,rdepth_str)
	call getarg(20,rfibre_str)
	call getarg(21,SLA_str)
	call getarg(22,LAIMAX_str)
	call getarg(23,LAIMIN_str)
	read(slat_str,*) slat
	read(co2ca_str,*) co2ca
	read(ioput_str,*) ioput
	read(a1_str,*) a1
	read(Ds0_str,*) Ds0
	read(Vcmx0_str,*) Vcmx0
	read(extkU_str,*) extkU
	read(xfang_str,*) xfang
	read(alpha_str,*) alpha
	read(stom_n_str,*) stom_n
	read(wsmax_str,*) wsmax
	read(wsmin_str,*) wsmin
	read(rdepth_str,*) rdepth
	read(rfibre_str,*) rfibre
	read(SLA_str,*) SLA
	read(LAIMAX_str,*) LAIMAX
	read(LAIMIN_str,*) LAIMIN
      return
      end
