...
def getModelINP(RUN_ID,Model_ID,ConnSTR ):
...
#    connSTR1= U'eco/b00mer@129.15.138.13:1521/oubcf'
    ...
    try:
        ...
        c2 = conn.cursor()
        prm = conn.cursor()
        prm.execute(U"Select PNAME,PVALUE From RT_PARAMETERS Where PARMA_TYPE=0 and MODEL_ID = 'TECO1'") #:1 ',(Model_ID,))
        wd =[]
        sql=''
        for par in prm:
        ...
        c1.callproc(U'' + sql,(RUN_ID,c2)) #TECO_INP_MOD_ID',(RUN_ID,c2))
        for p in c2:
            z=0
            row =''
            for i in p:
