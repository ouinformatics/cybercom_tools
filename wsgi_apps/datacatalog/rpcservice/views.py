...
def rtVariable(row={},**kwargs):
    ...
    try:
        ...
        return str(e)
#@rpcmethod(name='catalog.getTableData',signature=['Result:JSON Success/Errors', 'Param: tablename=<string>, column =[list of column names]'])
#def getMetadata(tablename, column=[],**kwargs):
#    '''
#    Retrieve Data from specific Table.
#        
#    Parameters:
#        tablename - required.
#        column - list for columns to return.
#    Returns
#        JSON Success/Errors
#    
#    '''
...
def getMetadata1(event_id,column=['var_id','result_text'] ,**kwargs):
    '''
    ...
    try:
        ...
        return "event_id must be an integer"
    res = {}
    try:
        md=datalayer.Metadata()
        sWhere = 'event_id = %d' % (event_id) 
        res = md.Search('dt_result',where=sWhere,column=column)
        return res
    except Exception as e:
        return str(e)
@rpcmethod(name='catalog.getEventMetadata',signature=['Result:JSON Success/Errors', 'Param: int cat_id'])
def getMetadataEvent(cat_id,column=['event_id','event_name'], **kwargs):
    '''
    Retrieve Metada Query for specific Catalog ID.
    ...
    try:
        ...
        return "Cat_id must be an integer"
    res = {}
    try:
        md=datalayer.Metadata()
        sWhere = 'cat_id = %d' % (cat_id) 
        res = md.Search('dt_event',where=sWhere,column=column)
        return res
    except Exception as e:
        return str(e)
@rpcmethod(name='catalog.getCatalogMetadata',signature=['Result:JSON Success/Errors', 'Param: int commons_id'])
def getMetadataCat(commons_id,column=['cat_id','cat_name'] ,**kwargs):
    '''
    ...
    try:
        ...
        return "Commons_id must be an integer"
    res = {}
    try:
        md=datalayer.Metadata()
        sWhere = 'commons_id =' + str(commons_id)
        res = md.Search('dt_catalog',where=sWhere,column=column)
        return res
    except Exception as e:
        return str(e) + " this is an error"
