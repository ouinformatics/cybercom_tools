...
findFiles() {
    ...
    if [ -z $VRT ]; then
        ...
        find ${DATAPATH}/tile*/${PRODUCT}/ -name *.gtiff| cut -d'.' -f2,3 |sort | grep '^[0-9]\{8\}' | uniq -c | grep '^      8 ' | sed 's/^      8 //g'
    else
        # Check if VRT has been built
        find ${DATAPATH}/mosaic/${PRODUCT}/ -name *.vrt| sed 's/_aea//g' | cut -d'.' -f2,3 | sort | grep '^[0-9]\{8\}' | uniq
    fi
}
